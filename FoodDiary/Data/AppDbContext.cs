...
namespace FoodDiary.Data
{
    ...
    public class AppDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // Relacja: DailySummary -> User (wiele podsumowań do jednego użytkownika)
            modelBuilder.Entity<DailySummary>()
                .HasOne(ds => ds.User)
                .WithMany(u => u.DailySummaries)
                .HasForeignKey(ds => ds.UserId);

            // Relacja: Meal -> DailySummary (wiele posiłków do jednego podsumowania)
            modelBuilder.Entity<Meal>()
                .HasOne(m => m.DailySummary)
                .WithMany(ds => ds.Meals)
                .HasForeignKey(m => m.DailySummaryId);

            // Relacja: MealEntry -> FoodItem (wiele wpisów do jednego produktu)
            modelBuilder.Entity<MealEntry>()
                .HasOne(me => me.FoodItem)
                .WithMany(fi => fi.MealEntries)
                .HasForeignKey(me => me.FoodItemId);

            // Relacja: MealEntry -> Meal (wiele wpisów do jednego posiłku, kaskadowe usuwanie)
            modelBuilder.Entity<MealEntry>()
                .HasOne(me => me.Meal)
                .WithMany(m => m.MealEntries)
                .HasForeignKey(me => me.MealId)
                .OnDelete(DeleteBehavior.Cascade);

            // Relacja: FoodItem -> FoodCategory (wiele produktów do jednej kategorii, kategoria opcjonalna)
        ...
        private void SeedData(ModelBuilder modelBuilder)
        {
            ...
            // Przykładowe wpisy posiłków dla użytkownika 1
            var mealEntriesUser1 = new List<MealEntry>
            {
            ...
            modelBuilder.Entity<MealEntry>().HasData(mealEntriesUser1);
            // Przykładowy wpis posiłku dla użytkownika 2
            var mealEntryUser2 = new MealEntry { MealEntryId = 5, MealId = 3, FoodItemId = 5, Quantity = 200 };
            modelBuilder.Entity<MealEntry>().HasData(mealEntryUser2);
        }
    }
}
